...
def nw_chart():
    ...
    with col1:
        ...
        num2_chart = st.number_input('结点初始连接数', min_value=2, value=4, step=2, key='input_2')
    with col2:
        num3_chart = st.number_input('重新连接尝试次数', min_value=1, value=4, step=1, key='input_3')
        num4_chart = st.number_input('重新连接成功概率', min_value=0.0, max_value=1.0, value=0.9, step=0.01, key='input_4')
